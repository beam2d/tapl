...
impl Ty {
    fn subtype(&self, other: &Ty) -> bool {
        match (self, other) {
            _ if self == other => true,
            (Ty::Arrow(s1, s2), Ty::Arrow(t1, t2)) => t1.subtype(s1) && s2.subtype(t2),
            (Ty::Record(s), Ty::Record(t)) => t.iter().all(|(key, t1)| match rcd_find(s, key) {
    ...
    fn join(&self, other: &Ty) -> Ty {
        match (self, other) {
            (Ty::Arrow(s1, s2), Ty::Arrow(t1, t2)) => match s1.meet(t1) {
                Some(m1) => Ty::Arrow(Box::new(m1), Box::new(s2.join(t2))),
                None => Ty::Top,
            },
            (Ty::Record(rcd_s), Ty::Record(rcd_t)) => {
                let mut rcd = Vec::new();
                for (k, s) in rcd_s {
                    if let Some((_, t)) = rcd_t.iter().find(|(l, _)| k == l) {
                        rcd.push((k.clone(), s.join(t)));
                    }
                }
                Ty::Record(rcd)
            }
    ...
    fn meet(&self, other: &Ty) -> Option<Ty> {
        match (self, other) {
            (Ty::Arrow(s1, s2), Ty::Arrow(t1, t2)) => match s2.meet(t2) {
                Some(m2) => Some(Ty::Arrow(Box::new(s1.join(t1)), Box::new(m2))),
                None => None,
            },
            (Ty::Record(rcd_s), Ty::Record(rcd_t)) => {
                let mut rcd = Vec::new();
                for (k, s) in rcd_s {
                    let ty = match rcd_t.iter().find(|(l, _)| k == l) {
                        Some((_, t)) => s.meet(t)?,
                        None => s.clone(),
                    };
                    rcd.push((k.clone(), ty));
                ...
                for (l, t) in rcd_t {
                    if rcd.iter().find(|(k, _)| k == l).is_none() {
                        rcd.push((l.clone(), t.clone()));
                    }
                }
                Some(Ty::Record(rcd))
            }
            (Ty::Bool, Ty::Bool) => Some(Ty::Bool),
            (s, Ty::Top) => Some(s.clone()),
            (Ty::Top, t) => Some(t.clone()),
            _ => None,
        }
    }
}
    ...
    fn shift(&self, d: i64) -> Node {
        fn walk(c: i64, d: i64, t: &Node) -> Node {
            let expr = match t.expr.as_ref() {
                ...
                Expr::If(t1, t2, t3) => Expr::If(walk(c, d, t1), walk(c, d, t2), walk(c, d, t3)),
    ...
    fn subst(&self, j: i64, s: &Node) -> Node {
        fn walk(j: i64, c: i64, s: &Node, t: &Node) -> Node {
            let expr = match t.expr.as_ref() {
                ...
                Expr::If(t1, t2, t3) => {
                    Expr::If(walk(j, c, s, t1), walk(j, c, s, t2), walk(j, c, s, t3))
                }
                Expr::False => Expr::False,
                Expr::True => Expr::True,
            };
    ...
    fn eval1(&self) -> Result<Node> {
        match self.expr.as_ref() {
            ...
            Expr::If(cond, then, els) => match cond.expr.as_ref() {
                ...
                _ => Ok(Node::new(
                    self.span,
                    Expr::If(cond.eval1()?, then.clone(), els.clone()),
                )),
            },
    ...
    fn ty(&self, ctx: &mut Context) -> Result<Ty> {
        match self.expr.as_ref() {
            ...
            Expr::App(t1, t2) => {
                ...
                let ty2 = t2.ty(ctx)?;
                match ty1 {
                    Ty::Arrow(arg, ret) if ty2.subtype(&arg) => Ok(*ret),
                    Ty::Arrow(arg, _) => Err(type_mismatch(t2, &arg, &ty2)),
                    _ => Err(type_error(t1.span, "cannot apply non-abstract value")),
                }
            }
            Expr::Record(entries) => {
                let mut rcd = Vec::new();
            ...
            Expr::Proj(node, key) => match node.ty(ctx)? {
                Ty::Record(entries) => rcd_find(&entries, key)
                    .cloned()
                    .ok_or(type_error(self.span, "key not found in the record")),
                _ => Err(type_error(node.span, "cannot project non-record term")),
            },
            Expr::If(cond, then, els) => {
                ...
                Ok(then_ty.join(&els_ty))
            }
            Expr::False | Expr::True => Ok(Ty::Bool),
        }
    }
...
impl<'tx> Parser<'tx> {
    ...
    fn expr(&mut self) -> Result<Node> {
        ...
        while matches_kind!(
            self.src.peek(),
            TokenKind::BraceL | TokenKind::Lambda | TokenKind::ParenL | TokenKind::Ident
        ) {
            let rhs = self.arg()?;
        ...
        let mut node = match self.src.next() {
            Some(Ok(token)) => match token.kind {
                ...
                TokenKind::If => {
                    ...
                    let els = self.expr()?;
                    Node::at(token.span, Expr::If(cond, then, els))
                }
                TokenKind::Lambda => {
                ...
                TokenKind::False => Node::at(token.span, Expr::False),
                TokenKind::True => Node::at(token.span, Expr::True),
