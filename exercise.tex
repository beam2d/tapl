...
\begin{enumerate}
  \item If $|t| = \Ab x |t|_1$, it holds that
    ...
    By induction hypothesis, we have $\Cardi{FV(|t|_1)} \le \It{size}(|t|_1)$.
...
By induction hypothesis, it holds that \M{\TyRel{|t|'_1}{|Bool|}}.
...
It holds that $|T| = |Nat|$ and, by the inversion lemma,
  \M{\TyRel{|t|_1}{|Nat|}}.
We can derive \M{\TyRel{|t|'_1}{|Nat|}} by induction hypothesis.
...
\begin{itemize}
  ...
  \item \Case\Rule{E-If}:
    ...
    By induction hypothesis, it holds \M{\CtxTyRel{\Gamma}{|t|'_1}{|Bool|}}.
...
\begin{Proofs}
  \AxiomC{}
  \RuleLabel{S-RcdPerm}
  \UnaryInfC{$\PhantomRel\Sb \Rec{|x:Nat,y:Nat,z:Nat|}$}
  \noLine\UnaryInfC{$\Sb \Rec{|y:Nat,x:Nat,z:Nat|}$}
  \AxiomC{}
  \RuleLabel{S-RcdWidth}
  \UnaryInfC{\Rec{|y:Nat,x:Nat,z:Nat|}}
  \noLine\UnaryInfC{$\Sb \Rec{|y:Nat|}$}
  \RuleLabel{S-Trans}
  \BinaryInfC{$\Sub{\Rec{|x:Nat,y:Nat,z:Nat|}}{\Rec{|y:Nat|}}$}
\end{Proofs}
...
Let \M{|t| = \Rec{\AbTy{|x|}{\Rec{}} |x,\ | \Rec{}}}.
We have \M{\CtxTyRel{}{|t|}{\CParen{\Rec{} \to \Rec{}} \times \Rec{}}}.
