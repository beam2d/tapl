...
\newcommand\Ab@Impl[1]{\Ab@One#1\texttt.\@ifnextchar\relax{}{\Ab@Impl}}
\newcommand\Ab@One[1]{\lambda\texttt{#1}}
\newcommand\Abn[1]{\Repeat{#1}{\lambda\texttt.}}
\newcommand\AbTy[2]{\lambda #1 \texttt{:} #2 \texttt{.}}
\newcommand\AlCtx[2]{#1 \Tsto #2}
\newcommand\AlTyRel[3]{\AlCtx{#1}{\TyRel{#2}{#3}}}
\newcommand\App{\CdSpc}
\newcommand\As[2]{#1\CdSpc\texttt{as}\CdSpc#2}
\newcommand\Bigstep{\Downarrow}
\newcommand\Cardi[1]{\vert#1\vert}
\newcommand\Case{\textit{Case} }
\newcommand\CaseArm[2]{#1 \To #2 \@ifnextchar\CaseArm{\Cd{\ \|\ }}{}}
\newcommand\CaseOf[1]{\Cd{case\ }#1\Cd{\ of\ }}
\newcommand\CBracket[1]{\texttt[#1\texttt]}
\newcommand\CdSpc{\phantom{\Cd.}}
...
\newcommand\Implies{\CdSpc\text{implies}\CdSpc}
\newcommand\Indices[1]{{}^{#1}}
\newcommand\It[1]{\textit{#1}}
\newcommand\LambdaNB{$\lambda$\textbf{NB}}
\newcommand\Lemma[1]{\textsc{Lemma #1.}}
\newcommand\Let[2]{\Cd{let\ } #1 = #2 \Cd{\ in\ }}
\newcommand\Lm{\lambda}
\newcommand\M[1]{\unskip\CdSpc\M@body{#1}}
\newcommand\M@body[1]{%
  $#1$%
  \@ifnextchar,{\M@SpaceAfter}{%
  \@ifnextchar.{\M@SpaceAfter}{%
  \@ifnextchar){\M@SpaceAfter}{%
  \@ifnextchar]{\M@SpaceAfter}{%
  \@ifnextchar\}{\M@SpaceAfter}{%
...
\newcommand\Mh[1]{\M@body{#1}}
\newcommand\M@SpaceAfter[1]{#1\phantom{\Cd.}\ignorespaces}
...
If \M{|t| = |succ s|} for some term |s|,
  the last rule used to derive both $|t| \to |t|'$
  and $|t| \to |t|''$ is \Rule{E-Succ}.
We then have \M{|t|' = |succ s|'} for some term $|s|'$,
  and the statement \M{|succ s| \to |succ s|'} is derived from
  $|s| \to |s|'$.
Similarly, we have \M{|t|'' = |succ s|''} for some term $|s|''$,
  and the statement \M{|succ s| \to |succ s|''} is derived from
  $|s| \to |s|''$.
We now have $|s| \to |s|'$ and $|s| \to |s|''$,
  and then we get $|s|' = |s|''$ by the induction hypothesis.
It implies \M{|t|' = |succ s|' = |succ s|'' = |t|''}.

If \M{|t| = |pred s|} for some term |s|,
...
To see this,
  we need to check if the left hand side \M{|pred t|_1} of the statement
  derived by \Rule{E-Pred} does not conflict with the left hand side
  \M{|pred succ nv|_1} of \Rule{E-PredSucc}.
Assume they conflict.
We then, for some $|t|'_1$,
  have \M{|pred succ nv|_1 \to |pred t|'_1}
  derived by \Rule{E-Pred}.
The premise of this one-step derivation is \M{|succ nv|_1 \to |t|'_1}.
...
If \M{|t| = |iszero s|} for some term |s|,
  a similar discussion is applied as the previous case of \M{|t| = |pred s|},
...
By applying \Rule{B-IfTrue},
  we get \M{\If{|t|_1}{|t|_2}{|t|_3} \Bigstep |v|}.
...
Next, we apply \Rule{E-IfTrue} to the right hand side to obtain
  \M{\If{|t|_1}{|t|_2}{|t|_3} \to^* |t|_2}.
Combining it with $|t|_2 \to^* |v|$,
  we get \M{\If{|t|_1}{|t|_2}{|t|_3} \to^* |v|}.
...
Full evaluation of |prd| |c|$_n$ involves applications of |ss|
  $n$ times starting from \M{|pair c|_0|\ c|_0} and selecting |fst|.
...
\begin{enumerate}
  ...
  \item Otherwise, |t| is written as \M{|t| = |t|_1 |\ t|_2}.
...
\begin{itemize}
  ...
  \item Otherwise, we can write \M{|t| = \CParen{|s|_1 \App |s|_2}}
...
\begin{enumerate}
  ...
  \item If $|t|_1, |t|_2 \in \mathcal S_n$,
    ...
    By definition, we have \M{\CParen{|t|_1 \App |t|_2} \in S_{n, i + 1} \subseteq \mathcal S_n}.
\end{enumerate}
\qed
...
If |t| is written as \M{|t| = |t|_1 \App |t|_2},
  $|t|_1$ and $|t|_2$ are both $n$-terms.
By the induction hypothesis,
  $\DShift_c^d(|t|_1)$ and $\DShift_c^d(|t|_2)$ are $(n + d)$-terms.
Therefore,
  \M{\DShift_c^d(|t|_1 \App |t|_2) = \DShift_c^d(|t|_1) \CdSpc \DShift_c^d(|t|_2)}
  is an $(n + d)$-term. \qed
...
Otherwise, we can write \M{|t| = |t|_1 |\ t|_2}.
...
Otherwise, \M{|t| = |t|_1 |\ t|_2}. We have
\begin{align*}
...
\Case\textsc{E-IfTrue}:
We have \M{|t| = \If{|true|}{|t|_1}{|t|_2} \to |t|_1}.
By the inversion lemma, we have \M{\TyRel{|t|_1}{|T|}}.
\textsc{E-IfFalse} case is similar.

\Case\textsc{E-If}:
We have \M{|t| = \If{|t|_1}{|t|_2}{|t|_3}}
  and \M{|t|' = \If{|t|'_1}{|t|_2}{\break|t|_3}}
  where $|t|_1 \to |t|'_1$.
Since \M{\TyRel{|t|}{|T|}},
  we have \M{\TyRel{|t|_1}{|Bool|}}, \M{\TyRel{|t|_2}{|T|}},
  and \M{\TyRel{|t|_3}{|T|}} by the inversion lemma.
By the induction hypothesis, it holds that \M{\TyRel{|t|'_1}{|Bool|}}.
We thus conclude \M{\TyRel{|t|'}{|T|}}.

\Case\textsc{E-Succ}:
We have \M{|t| = |succ t|_1} and \M{|t|' = |succ t|'_1}
  where $|t|_1 \to |t|'_1$.
It holds that $|T| = |Nat|$ and, by the inversion lemma,
  \M{\TyRel{|t|_1}{|Nat|}}.
We can derive \M{\TyRel{|t|'_1}{|Nat|}} by the induction hypothesis.
Therefore, we conclude \M{\TyRel{|t|'}{|Nat|}}.
We can apply similar discussions to \Rule{E-Pred} and \Rule{E-IsZero}.

\Case\textsc{E-PredZero}:
It is obvious that \M{\TyRel{|pred 0|}{|Nat|}} and \M{\TyRel{|0|}{|Nat|}}.
...
Simply removing \Rule{E-PredZero} breaks the progress property
  because, by \M{\TyRel{|0|}{|Nat|}} and \Rule{T-Pred},
  we have \M{\TyRel{|pred 0|}{|Nat|}},
  which could not be evaluated anymore.

\Ex{8.3.6}
This property does not hold.
For example, let \M{|t| = |if true then 0 else true|},
  \break then \M{|t| \to |0|} and \M{\TyRel{|0|}{|Nat|}},
...
\begin{itemize}
  \item If |t| is a well typed term, then |t| is evaluated to a value |v|, i.e.,
        $|t| \Bigstep |v|$.
  \item If \M{\TyRel{|t|}{|T|}} and \M{|t| \Bigstep |v|}, then \M{\TyRel{|v|}{|T|}}.
\end{itemize}
...
2. Let \M{b = \TyRel{|f|}{|Bool| \to |Bool|}} and \M{\Gamma = (b, \TyRel{|x|}{|Bool|})}.
\begin{Proofs} \small
  \AxiomC{$b \in \Gamma$}
  \RuleLabel{T-Var}
  \UnaryInfC{$\Ctx{\Gamma}{b}$}
...
Assume, for contradiction, that there exists a context $\Gamma$ and a type |T|
  such that \M{\CtxTyRel{\Gamma}{|x x|}{|T|}}.
By the inversion lemma,
  there is a type |S| that satisfies
  $\CtxTyRel{\Gamma}{|x|}{|S| \to |T|}$ and \M{\CtxTyRel{\Gamma}{|x|}{|S|}}.
By the inversion lemma again,
  it holds that \M{\TyRel{|x|}{|S| \to |T|} \in \Gamma} and
  \M{\TyRel{|x|}{|S|} \in \Gamma}.
...
\Ex{9.3.9}
We show it by induction on derivations of \M{\CtxTyRel{\Gamma}{|t|}{|T|}}.
...
\Case\Rule{T-If}:
In this case, we can write \M{|t| = \If{|t|_1}{|t|_2}{|t|_3}}
  with subderivations of
  \M{\CtxTyRel{\Gamma}{|t|_1}{|Bool|}},
  \M{\CtxTyRel{\Gamma}{|t|_2}{|T|}}, and
  \M{\CtxTyRel{\Gamma}{|t|_3}{|T|}}.
...
\begin{itemize}
  \item \Case\Rule{E-IfTrue}:
    ...
    We then immediately obtain \M{\CtxTyRel{\Gamma}{|t|'}{|T|}}.
  \item \Case\Rule{E-IfFalse}: Similar as \Rule{E-IfTrue} case.
  \item \Case\Rule{E-If}:
    In this case, we have $|t|_1 \to |t|'_1$
      and $|t| \to \If{|t|'_1}{|t|_2}{|t|_3} = |t|'$.
    By the induction hypothesis, it holds \M{\CtxTyRel{\Gamma}{|t|'_1}{|Bool|}}.
    Therefore, we get \M{\CtxTyRel{\Gamma}{|t|'}{|T|}}.
\end{itemize}
...
\Case\Rule{T-App}:
In this case,
  we can write \M{|t| = |t|_1 \App |t|_2}
  with subderivations of
  \M{\CtxTyRel{\Gamma}{|t|_1}{|S| \to |T|}} and
  \M{\CtxTyRel{\Gamma}{|t|_2}{|S|}} for some type |S|.
There are three cases of the evaluation rule that derives $|t| \to |t|'$.
\begin{itemize}
  \item \Case\Rule{E-App1}:
    In this case, we have $|t|_1 \to |t|'_1$ and \M{|t| \to |t|'_1 \App |t|_2}.
    By the induction hypothesis,
      it holds \M{\CtxTyRel{\Gamma}{|t|'_1}{|S| \to |T|}}.
    By applying the typing rule \Rule{T-App} to
      this typing relation and \M{\CtxTyRel{\Gamma}{|t|_2}{|S|}},
      we get \M{\CtxTyRel{\Gamma}{|t|'}{|T|}}.
  \item \Case\Rule{E-App2}:
    We can prove for this case similarly as \Rule{E-App1} case.
  \item \Case\Rule{E-AppAbs}:
    In this case,
      we can write \M{|t| = \CParen{\AbTy{|x|}{|S|} |t|_{12}} |v|_2} and
      $|t|' = [|x| \mapsto |v|_2] |t|_{12}$.
    By the inversion lemma of typing relations,
      we have
      \M{\CtxTyRel{\Gamma}{\CParen{\AbTy{|x|}{|S|} |t|_{12}}}{|S| \to |T|}}
      and
      \M{\CtxTyRel{\Gamma}{|v|_2}{|S|}}.
    The former relation is derived by \Rule{T-Abs},
      with which we have \M{\CtxTyRel{\Gamma, |x:S|}{|t|_{12}}{|T|}}.
    By the substitution lemma,
      we get \M{\CtxTyRel{\Gamma}{[|x| \mapsto |v|_2] |t|_{12}}{|T|}},
...
\textbf{No}.
For example,
  let \M{\Gamma = \TyRel{|y|}{|Bool| \to |Bool|}}
  and \M{|t| = \CParen{\AbTy{|x|}{|Bool|} |x|} \App |y|}.
Applying \Rule{E-AppAbs} yields
  $|t| \to |t|' = [|x| \mapsto |y|] |x| = |y|$.
Using \Rule{T-Var}, we can derive \M{\CtxTyRel{\Gamma}{|t|'}{|Bool| \to |Bool|}}.
However, it is obvious that |t| is not typed under $\Gamma$,
  so \M{\CtxTyRel{\Gamma}{|t|}{|Bool| \to |Bool|}} does not hold.
...
\end{Proofs}
If \M{\CtxxTyRel E{\Gamma}{\AbTy{|_|}{|T|_1} |t|_2}{|T|_1 \to |T|_2}},
  it is derived from \M{\CtxxTyRel E{\Gamma}{|t|_2}{|T|_2}}.
By the induction hypothesis,
  it holds \M{\CtxxTyRel I{\Gamma}{e(|t|_2)}{|T|_2}}.
Choose a variable name |x| from the outside of $\Gamma$.
Then, weakining yields \M{\CtxxTyRel I{\Gamma, |x:T|_1}{e(|t|_2)}{|T|_2}},
  from which we can derive
  \M{\CtxxTyRel I{\Gamma}{\AbTy{|x|}{|T|_1} e(|t|_2)}{|T|_1 \to |T|_2}}.

If \M{\CtxxTyRel I{\Gamma}{\AbTy{|x|}{|T|_1} e(|t|_2)}{|T|_1 \to |T|_2}}
  where $|x| \notin \Gamma$ and $|x| \notin \FV(|t|_2)$,
  it is derived from \M{\CtxxTyRel I{\Gamma, |x:T|_1}{e(|t|_2)}{|T|_2}}.
By the induction hypothesis,
  it holds \M{\CtxxTyRel E{\Gamma, |x:T|_1}{|t|_2}{|T|_2}}. \break
Since |x| does not appear in $|t|_2$ as a free variable,
  we can omit it from the context as \M{\CtxxTyRel E{\Gamma}{|t|_2}{|T|_2}},
  from which we can derive
  \M{\CtxxTyRel E{\Gamma}{\AbTy{|_|}{|T|_1} |t|_2}{|T|_1 \to |T|_2}}.
...
If $|t| \to_E |t|'$,
  ...
  \M{|t| = \CParen{\AbTy{|_|}{|T|_{11}} |t|_{12}} \App \break |v|_2},
...
If $e(|t|) \to_I e(|t|')$,
  ...
  \M{|t| = \CParen{\AbTy{|_|}{|T|_{11}} |t|_{12}} \App |v|_2}.
In this case,
  we have \M{e(|t|) = \CParen{\AbTy{|x|}{|T|_{11}} e(|t|_{12})} \App |v|_2}
...
\begin{itemize}
  \item[(1)]
    We can define a derived form as
      \M{|t as T| \EqDef \CParen{\AbTy{|x|}{|T|} |x|} \App |t|}.
...
\begin{enumerate}
  ...
  \item Type preservation for \Rule{E-Let} is straight-forward.
    ...
    \Lemma{11.8.2.1}
    If \M{\CtxTyRel{\Gamma}{|v|}{|S|}},
      \M{\TyRel{|p|}{|S|} \To \Gamma'},
      and \M{\CtxTyRel{\Gamma, \Gamma'}{|t|}{|T|}},
      then \M{\CtxTyRel{\Gamma}{[\It{match}(|p|, |v|)] |t|}{|T|}}. \qed

    The lemma is shown by induciton on derivation of \M{\TyRel{|p|}{|S|} \To \Gamma'}.

    Type progress is shown by induction on derivation of the type of the term.
    Suppose we have a typing relation
      \M{\CtxTyRel{\Gamma}{\Let{|p|}{|t|_1} |t|_2}{|T|_2}}.
    Since it should be derived by \Rule{T-Let},
      we have \M{\CtxTyRel{\Gamma}{|t|_1}{|T|_1}} for some type $|T|_1$.
...
In this case,
  we can write \M{|t| = |isnil[S] (nil[T])|}.
Using \Rule{T-Nil} and \Rule{T-Isnil},
  we derive \M{\TyRel{|t|}{|Bool|}},
...
Using the inversion lemma for \Rule{T-Head},
  we obtain \M{\CtxTyRel{\Gamma}{|cons| \CBracket{|S|_2} \App |v|_1 \App |v|_2}{|List S|_1}}
  and $|T| = |S|_1$.
By applying the inversion lemma for \Rule{T-Cons},
  we further obtain \M{\CtxTyRel{\Gamma}{|v|_1}{|S|_1}}.
...
For example, given a value |a| of type |A|,
  applying a function \M{\AbTy{|f|}{|A| \to |A|} |\ f (f a)|} to
...
In this case, we have
  $|t| = \Rcd{|t|_1 |,t|_2}$,
  \M{\CtxTyRel{\Gamma}{|t|_1}{|T|_1}}, and
  \M{\CtxTyRel{\Gamma}{|t|_2}{|T|_2}}.
By the induction hypothesis,
...
\Case\Rule{T-Proj1}:
In this case, we have
  \M{|t| = |t|_1 |.1|} and \M{\CtxTyRel{\Gamma}{|t|_1}{|T|_{11} \times |T|_{12}}}.
By the induction hypothesis,
...
The precedence of |!| is higher than function applications,
  i.e., \M{|!x y|} is equivalent to \M{|(!x) y|}.
...
\Ex{13.1.3}
Define an operator \M{|free r|} that takes a reference and frees its storage,
...
Let
  $\Gamma = \emptyset$,
  \M{\mu = (l \mapsto \AbTy{|x|}{|Unit|} \CParen{|!|l} \App |x|)},
...
Let \M{|t| = \Rcd{\AbTy{|x|}{\Rcd{}} |x,\ | \Rcd{}}}.
We have \M{\CtxTyRel{}{|t|}{\CParen{\Rcd{} \to \Rcd{}} \times \Rcd{}}}.
With the rule \Rule{S-ProdWidth},
  we can derive $\CtxTyRel{}{|t|}{|\{\}| \to |\{\}|}$.
Therefore, a term \M{|t \{\}|} is well-typed, which is obviously a stuck state.
...
With the above rule, we can derive \M{\CtxTyRel{}{|f|}{|Top| \to |Unit|}}.
Therefore, we have
  \M{\CtxTyRel{}{|f| \App \CParen{\AbTy{|x|}{|Top|} |unit|}}{|Unit|}}.
However,
  this term is evaluated as \M{|f| \App \CParen{\AbTy{|x|}{|Top|} |unit|} \to |0|},
...
Let \M{|v as T| \to |v|} be derived by \Rule{E-Downcast}.
By the premise, we have \M{\CtxTyRel{}{|v|}{|T|}}.
By \Rule{T-Ascribe}, \M{\CtxTyRel{}{|v as T|}{|T|}}.
Therefore, the type is preserved by the evaluation step \M{|v as T| \to |v|}. \qed
...
A similar transformation is applied when the record items are permutated so that
  \Rule{T-Sub} is applied to \M{\CtxTyRel{\Gamma}{|t|_j}{|T|_j}} with $j \neq 1$.
...
\Ex{16.2.3}
Let \M{|s| = \CParen{\AbTy{|x|}{|Top|} |x|} \App \Rcd{}} and $|t| = \Rcd{}$.
...
We have $|t| = \AbTy{|x|}{|T|_1} |t|_2$ and $|T| = |T|_1 \to |T|_2$
  where the premise gives \M{\CtxTyRel{\Gamma, |x:T|_1}{|t|_2}{|T|_2}}.
By the induction hypothesis,
  the premise implies \M{\AlTyRel{\Gamma, |x:T|_1}{|t|_2}{|S|_2}}
  where $\Sub{|S|_2}{|T|_2}$.
Using \Rule{TA-Abs},
  we obtain \M{\AlTyRel{\Gamma}{|t|}{|T|_1 \to |S|_2}}.
Since $\Sub{|T|_1 \to |S|_2}{|T|_1 \to |T|_2}$ by \Rule{S-Refl} and \Rule{S-Arrow},
  the conclusion holds.

\Case\Rule{T-App}:
We have \M{|t| = |t|_1 \App |t|_2} and $|T| = |T|_{12}$
  where the premises give \M{\CtxTyRel{\Gamma}{|t|_1}{|T|_{11} \to |T|_{12}}} and
  \M{\CtxTyRel{\Gamma}{|t|_2}{|T|_{11}}}.
By the induction hypothesis,
  the premises imply
...
Therefore, using \Rule{TA-App},
  we derive \M{\AlTyRel{\Gamma}{|t|_1 \App |t|_2}{|S|_{12}}}.
Since $\Sub{|S|_{12}}{|T|_{12}}$, this is the conclusion we wanted.

\Case\Rule{T-Sub}:
The first premise with the induction hypothesis gives \M{\AlTyRel{\Gamma}{|t|}{|T|}}.
...
We have $|t| = \Rcd{|l|_i |=t|_i \Indices{i\in1..n}}$ and
  ...
  where the premises give \M{\CtxTyRel{\Gamma}{|t|_i}{|T|_i}} for all $i$.
By the induction hypothesis,
  \M{\AlTyRel{\Gamma}{|t|_i}{|S|_i}} with $\Sub{|S|_i}{|T|_i}$ for all $i$.
Using \Rule{TA-Rcd},
  we obtain \M{
...
We have $|t| = |t|_1 |.l|_{i_1}$ and $|T| = |T|_{i_1}$
  ...
  \M{\CtxTyRel{\Gamma}{|t|_1}{\Rcd{|l|_i |:T|_i \Indices{i\in1..n}}}}.
By the induction hypothesis,
  \M{\AlTyRel{\Gamma}{|t|_1}{|S|_1}}
  with \M{\Sub{|S|_1}{\Rcd{|l|_i |:T|_i \Indices{i\in1..n}}}}. \break
...
By \Rule{TA-Proj},
  \M{\AlTyRel{\Gamma}{|t|_1 |.l|_{i_1}}{|S|_{j_1}}} is derived.
