...
\Ex{21.2.2}
Define $\Uc$ and $F$ as follows.
\begin{align*}
  \Uc &= \Set{|T| \in \Set{1, 2}^* \Pto \Set{\to, \times, |Top|}
...
Proof sketch of $\mu F = \Tc_f$ and $\nu F = \Tc$:

Since $\Tc_f$ is $F$-closed, $\mu F \subseteq \Tc_f$ by the principle of induction.
...
Therefore, |T| belongs to $F^n(\emptyset)$ for some finite $n$,
  i.e, $|T| \in \mu F$, and thus $\mu F = \Tc_f$.

We also have that $\Tc$ is $F$-consistent,
  and thus $\Tc \subseteq \nu F$ by the principle of coinduction.
If $|T| \in \Uc \setminus \Tc$,
...
\Ex{21.3.8}
Let $D = \Set{(|T|, |T|) \mid |T| \in \Tc}$.
To show that $D$ is $S$-consistent,
  let $|T| \in \Tc$ and show that $(|T|, |T|) \in S(D)$
...
\Ex{21.4.2}
{ \newcommand\TR{\It{TR}}
For any $(x, y) \in \Uc \times \Uc$,
  we can take $z = x$ to have $(x, z), (z, y) \in \Uc \times \Uc$.
Therefore, $\TR(\Uc \times \Uc) = \Uc \times \Uc$,
  which implies $F^\TR(\Uc \times \Uc) = \Uc \times \Uc$.
That is, $\Uc \times \Uc$ is the largest fixed point of $F^\TR$. \qed
}

\Ex{21.5.2}
Let $x \in \Tc_f \times \Tc_f$ and $X \subseteq \Tc_f \times \Tc_f$.
There are four cases of the form of $x$.

\Case$x = (|T|, |Top|)$ where $|T| \in \Tc_f$ \\
...
\[
  ...
  \begin{cases}
    ...
    ~ \Set{(|S|_1, |T|_1), (|S|_2, |T|_2)} &
      ...
      (|S|_1, |T|_1), (|S|_2, |T|_2) \in \mu\Tc_f \\
    ~ \Set{(|T|_1, |S|_1), (|S|_2, |T|_2)} &
      x = (|S|_1 \to |S|_2, |T|_1 \to |T|_2), ~
      (|T|_1, |S|_1), (|S|_2, |T|_2) \in \mu\Tc_f \\
    ~ \Up & \text{otherwise} \\
  \end{cases}
\]

By replacing $\Tc_f$ with $\Tc$,
...
If $\Support(X) \Up$,
  then $X \not\subseteq F(Y)$ for any $Y \subseteq \Uc$.
...
Therefore, if $F$ has no loop and a finite depth
  ...
  $\Lfp_F(X)$ terminates for any finite $X \subseteq \Uc$.
}
